...
const con = mysql.createConnection({
  ...
  password: process.env.DB_PASSWORD || 'sql@12345g',
  database: process.env.DB_DBNAME || 'employee_system',
});
  ...
  con.query(sql, [req.body.email], (err, result) => {
    ...
    if (result.length > 0) {
      ...
      bcrypt.compare(req.body.password, user.password, (err, isMatch) => {
        if (err) {
          return res.json({ Status: 'Error', Error: 'Error in comparing password' });
        }
        if (isMatch) {
          const id = user.id;
...
app.post('/createTask', (req, res) => {
  const sql = 'INSERT INTO `employee_system`.`tasks` (`taskname`,`assigned_to`,`status`) VALUES (?,?,?)';
  con.query(sql, [req.body.taskname, req.body.assignedTo,req.body.status], (err, result) => {
